...
export function CommissionSettings() {
  ...
  const filteredTours = useMemo(() => {
    ...
    if (searchTerm.trim()) list = list.filter(t => t.nome.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleUpdateTourPrice = async (tourId: string, newPrice: number) => {
    ...
    setLoading(true);
    try {
      await updateDoc(doc(db, 'tours', tourId), { precoBase: newPrice, updatedBy: user?.id, updatedAt: Timestamp.now() });
  ...
  const handleUpdateTourCommission = async (tour: Tour, newValue: number) => {
    ...
    setLoading(true);
    try {
      await updateDoc(doc(db, 'tours', tour.id), { comissaoPadrao: newValue, updatedBy: user?.id, updatedAt: Timestamp.now() });
  ...
  const handleToggleTourActive = async (tour: Tour) => {
    setLoading(true);
    try {
      await updateDoc(doc(db, 'tours', tour.id), { ativo: tour.ativo === false ? true : false, updatedBy: user?.id, updatedAt: Timestamp.now() });
      showToast(tour.ativo === false ? '✅ Passeio ativado!' : '⚠️ Passeio desativado!', 'success');
      refreshData();
    } catch { showToast('Erro ao alterar status', 'error'); }
    finally { setLoading(false); }
  };

  const handleCreateTour = async (data: { nome: string; precoBase: number; comissaoPadrao: number }) => {
    setLoading(true);
    try {
  ...
  const handleCreatePromotion = async (data: { passeioId: string; valor: number; dataInicio: string; dataFim: string }) => {
    setLoading(true);
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-5">
        <div className="flex flex-col sm:flex-row justify-between gap-3 mb-4">
          <h3 className="font-semibold text-gray-800">🎫 Passeios Cadastrados ({filteredTours.length})</h3>
          <div className="flex gap-2">
            <button onClick={() => setShowInactive(s => !s)} className={`text-xs border rounded-lg px-2.5 py-1.5 ${showInactive ? 'bg-gray-200 dark:bg-gray-600' : 'text-gray-400'}`}>
              {showInactive ? '👁 Mostrar ativos' : '👁 Ver inativos'}
            </button>
            <button onClick={() => setShowTourModal(true)} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm">+ Novo Passeio</button>
          </div>
        </div>

        {(tours || []).length > 5 && (
          <input type="text" placeholder="🔍 Buscar passeio..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm mb-3 dark:bg-gray-700" />
        )}

        {filteredTours.length === 0 ? (
          <p className="text-center text-gray-400 py-8">Nenhum passeio encontrado</p>
        ) : (
          <ToursTable tours={filteredTours} loading={loading} onUpdateName={handleUpdateTourName} onUpdatePrice={handleUpdateTourPrice} onUpdateCommission={handleUpdateTourCommission} onToggleActive={handleToggleTourActive} />
        )}
      </div>

      <PromotionsList activePromos={activePromos} expiredPromos={expiredPromos} tours={tours || []} onDelete={handleDeletePromotion} onOpenModal={() => setShowPromoModal(true)} />
